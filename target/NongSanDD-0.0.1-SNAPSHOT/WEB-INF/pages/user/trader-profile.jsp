...
<link rel="stylesheet" href="/NongSanDD/resources/css/profile.css">
<link rel="stylesheet"
	href="/NongSanDD/resources/vendor/jquery-barrating/fontawesome-stars.css">
<link rel="stylesheet"
	href="/NongSanDD/resources/vendor/jquery-barrating/fontawesome-stars-o.css">
	...
	<section class="about-story">
		<div class="container">
			<div class="row">
						...
						<div class="icon-box">
							<div class="single-item">
								...
								<div class="count name color1">
									Tuổi:
									<c:if test="${trader.age > 0 }">${trader.name }</c:if>
								</div>
								<div class="name color1">
									SDT: <span style="color: #333"> ${trader.phoneNum }</span>
								</div>
							</div>
							<div class="single-item">
								<div class="icon">
				...
				<button onclick="writeCmt(${trader.id})" class="btn btn-primary"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal form-label-left" id="ratingForm"
						action="/NongSanDD/NguoiDung/them-nhan-xet" method="post">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal form-label-left" id="editRatingForm"
						action="/NongSanDD/NguoiDung/cap-nhap-nhan-xet" method="post">
...
<script type="text/javascript"
	src="/NongSanDD/resources/vendor/jquery-barrating/jquery.barrating.js"></script>

<script type="text/javascript">
	function editCmt(id, count, content){
		$("#cmtID").val(id);
		$("#editRatingCount").val(count);
		$("#editContent").val(content);
		
		$("#edit-rating").barrating({
			theme: 'fontawesome-stars',
			initialRating: count,
			onSelect: function(value, text) {
	               $("#editRatingCount").val(value);
	           }
		});
	...
	function updateCmtForm(){
		if($('#editRatingForm').valid()){
...
function writeCmt(id){
	$.ajax({
		...
		url : "/NongSanDD/NguoiDung/kiem-tra-them-cmt",
...
$(function() {
	$('#sum-start-rating').barrating({
		...
		initialRating: ${trader.ratingSum},
		readonly: true
	});
});
...
function initialize() {
	...
	$('#pac-input').val("${user.address.address}");
