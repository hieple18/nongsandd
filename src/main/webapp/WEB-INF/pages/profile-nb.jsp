...
<div class="container">
	<form:form class="form-horizontal form-label-left" id="register_form"
		modelAttribute="trader" action="/NongSanDD/TempNB/cap-nhap-tt" style="padding-top: 50px">
		...
		<input type="hidden" name="address.address" id="map_address" value="${trader.address.address }"/>
		<input type="hidden" name="address.lat" id="map_lat" value="${trader.address.lat }"/>
		<input type="hidden" name="address.lng" id="map_lng" value="${trader.address.lng }"/> 
		<input type="hidden" name="address.id" value="${trader.address.id }"/>
		<div class="item form-group">
			...
			<div class="col-md-2 col-sm-6 col-xs-12">
				<form:select id="select-commune" name="communeID" path="commune"
					...
					<c:forEach var="commune" items="${communes}">
						<option <c:if test="${commune.communeID == trader.commune}">selected="selected"</c:if>
...
<script>
var updateFlag = true;
var previousAddress = "${trader.address.address}";
var mapZoom = 15;
var currentLocation = {
